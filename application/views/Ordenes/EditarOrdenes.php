...
<div class="card-panel">
<?php
  echo form_open('Ordenes/actualizar/'. $editado->getId(),array('id'=>'formulario'));
?>
...
<tr>
     ...
     <td><?= $detalle->MarcaModelo; ?></td>
     <td><?=$detalle->serial; ?></td>
     <td><?= $detalle->getServicio(); ?></td>
     <td><div class='center'><i style='color:red' class='small material-icons' onclick='BorrarProducto(this.parentNode.parentNode.parentNode.rowIndex)'>delete</i>
     <i style='color:orange' onclick='ModificarProducto( <?=$i; ?> ,this.parentNode.parentNode.parentNode)' class='small material-icons' data-position='bottom' data-delay=50 data-tooltip='Editar'>edit</i></div>
   </td>
</tr>
<?php
  $detaEquipo[] = $detalle->getIdEquipo();
  $detaServicio[] = $detalle->getServicio();
   $i = $i + 1;
  }
...
var equipos = <?=json_encode($detaEquipo);?>;
var servicios = <?=json_encode($detaServicio);?>;


</script>
<script type="text/javascript" src="<?php echo base_url();?>assets/js/Ordenes/EditarOrdenes.js"></script>
